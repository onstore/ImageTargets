...
- (void) setup3dObjects
{
    ...
    for (int i=0; i < [textures count]; i++)
    {
        ...
        obj3D.numVertices = predioNumVerts; //NUM_TEAPOT_OBJECT_VERTEX;
        obj3D.vertices = predioVerts;// teapotVertices;
        obj3D.normals = predioNormals;// teapotNormals;
        obj3D.texCoords = predioTexCoords;// teapotTexCoords;
        
        //obj3D.numIndices = bananaNumVerts;// NUM_TEAPOT_OBJECT_INDEX;
...
- (void)renderFrameQCAR
{
    ...
    for (int i = 0; i < state.getNumActiveTrackables(); ++i) {
        ...
        if (QCAR::GL_11 & qUtils.QCARFlags) {
            ...
            ShaderUtils::rotatePoseMatrix(90.0, 1.0, 0.0, 0.0);
            ...
            glVertexAttribPointer(vertexHandle, 3, GL_FLOAT, GL_FALSE, 0, (const GLvoid*) &obj3D.vertices[0]);
            glVertexAttribPointer(normalHandle, 3, GL_FLOAT, GL_FALSE, 0, (const GLvoid*) &obj3D.normals[0]);
            glVertexAttribPointer(textureCoordHandle, 2, GL_FLOAT, GL_FALSE, 0, (const GLvoid*) &obj3D.texCoords[0]);
            ...
            // set input data to arrays
            //glVertexPointer(3, GL_FLOAT, 0, obj3D.vertices);
            //glNormalPointer(GL_FLOAT, 0, obj3D.normals);
            //glTexCoordPointer(2, GL_FLOAT, 0, obj3D.texCoords);
